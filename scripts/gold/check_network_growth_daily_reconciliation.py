"""Reconcile Gold network growth daily giữa MinIO và ClickHouse."""
...
def read_gold_network_growth_daily(spark: SparkSession) -> DataFrame:
    """Đọc Gold network growth daily từ MinIO.

    Input chính là SparkSession đã cấu hình S3A.
    Output là DataFrame Gold network growth daily từ Parquet.
    """
    return spark.read.parquet(GOLD_NETWORK_GROWTH_DAILY_PATH)


def build_expected_metrics(gold_df: DataFrame) -> dict[str, int]:
    """Tính metrics kỳ vọng từ Gold network growth daily Parquet.
...
def read_clickhouse_metrics() -> dict[str, int]:
    """Đọc metrics thực tế từ ClickHouse network growth daily table."""
    metric_selects = ",\n".join(
...
def print_reconciliation(expected: dict[str, int], actual: dict[str, int]) -> None:
    ...
    if has_mismatch:
        raise SystemExit("Gold network growth daily reconciliation failed")

    print("Gold network growth daily reconciliation passed")


def main() -> None:
    """So sánh Gold network growth daily giữa MinIO và ClickHouse."""
    spark = create_spark_session("bluesky-check-gold-network-growth-daily")
